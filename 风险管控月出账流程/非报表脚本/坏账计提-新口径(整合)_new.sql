...
select * from IPD_IN.I_IN_B_750_DHZ_201712@DL_ODL_89 
...
Select A.SERV_ID,
       ...
       Sum(Case
           ...
           End) AMOUNT12
  From (Select *
          From IPD_IN.I_IN_B_750_DHZ_201712@DL_ODL_89 A
         Where A.STAT_MONTH = '201712'
          and (  
  ( nvl(dzj_flag,0) =0 and 
   nvl(glf_flag,0) =0  and pingan_flag=0 
   and ict_cw_Flag=0 and zx_cw_flag=0 and dl_cw_flag=0 and hyq_flag=1)
  or   
  (nvl(dzj_flag,0)=2   or nvl(glf_flag,0)=2
  or (nvl(dzj_flag,0)=0 and nvl(glf_flag,0)=0 and pingan_flag=0 and ict_cw_Flag=0 and zx_cw_flag=0 and dl_cw_flag=0
  and nvl(hyq_flag,0)=0 and nvl(ppm_amount,0)=0 )
  )
  ) 
  --and  months_between(to_date(stat_month, 'yyyymm'),     to_date(owe_month2, 'yyyymm'))  >12 
  and state ='5JA'
  ) A
...
Select B.AREA_CODE,
	...
	Left Join PU_INTF.I_IN_KG_SERV_GRID Partition(P201712) C
...
--drop table wt_serv_jthz_201712 purge;
CREATE TABLE PU_WT.wt_serv_jthz_201712
...
Select A.*,
	...
	Left Join PU_INTF.I_PRD_VIP_ACCT Partition(P201712) D
		On A.ACCT_ID = D.ACCT_ID
	 And TO_CHAR(D.EXP_DATE, 'YYYYMM') > '201712';
   
Select sum(jthz_amount) FROM PU_WT.wt_serv_jthz_201712;  ---31496554256 31806556396 29252245575 26816081179 22828875264
