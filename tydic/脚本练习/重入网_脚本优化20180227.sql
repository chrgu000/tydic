...
Select /*+parallel(t,8)*/
        T.SERV_ID,
        T.LOCAL_CODE
        From PU_MODEL.TB_BIL_SERV_OWE_TRACK_201801@DL_EDW_YN T
       Where DATE_NO = TO_CHAR(LAST_DAY(TO_DATE('201801','YYYYMM')),'YYYYMMDD')--上月最后一天
         And IS_M_ETS_FLAG = 1
         And Substr(STATE_TR, -1) In ('A', 'D', 'C', 'F') ;


--SELECT COUNT(*) FROM pu_wt.tmp2_chrw_stream_info PARTITION (P0691)
