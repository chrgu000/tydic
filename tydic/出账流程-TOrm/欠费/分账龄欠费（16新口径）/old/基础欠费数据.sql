--历史欠费账龄表
--待列账
DROP TABLE TMP_OWE_20170911_DL PURGE;
CREATE TABLE TMP_OWE_20170911_DL  AS
select
     a.serv_id,
     ...
     a.billing_cycle_id 
 from pu_model.TB_BIL_OWE_TAB_201708 a
...
--- 销帐后
DROP TABLE TMP_OWE_20170911_XZH PURGE;
create table TMP_OWE_20170911_XZH as
Select A.*,
       Case
       ...
       Case
         When Nvl(B.DL_AMOUNT, 0) <> 0 And
              B.FEE_MONTH = '201708' Then
          1
         Else
          0
       End IS_M_DL_FLAG
  From PU_MODEL.TB_BIL_OWE_M_201708 A
  Left Join TMP_OWE_20170911_DL B
 ...
 --销账前欠费
DROP TABLE TMP_OWE_20170911_XZQ PURGE;
create table TMP_OWE_20170911_XZQ as
Select A.*,
       Case
         When A.STATE In ('5JD', '5JF') Then
          A.AMOUNT
       ...
       Case
         ...
         Else
          0
       End IS_M_DL_FLAG
  From PU_MODEL.TB_BIL_OWE_BEF_M_201708 A
  Left Join TMP_OWE_20170911_DL B
    On A.SERV_ID = B.SERV_ID
   And A.BILLING_CYCLE_ID = B.BILLING_CYCLE_ID
 Where A.STATE In ('5JA', '5JD', '5JC', '5JF'); 
